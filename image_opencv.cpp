...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    ...
    try {
        ...
        for (i = 0; i < num; ++i) { 
            ...
            int k = -1;
            //k=0:cavity
            //k=1:tree for use in if (class_id) part
            for (j = 0; j < classes; ++j) {
                ...
                if (dets[i].prob[j] > thresh && show) {
                    if (class_id < 0) {
                        ...
                        strcat(labelstr, buff);
                        /*sprintf(buff, ":%d", pix_width);
                        strcat(labelstr, buff);*/
                        printf("%d: %s: %.0f%% \n", j, names[j], dets[i].prob[j] * 100);
                        
                        strcpy(type, names[j]); //have to do this to copy the value of names into type, then type will be used below in line 970
                        k = j;
            ...
            if (class_id >= 0) {
                ...
                if (k==0){
                    ...
                    c_width = b_width;
                    //sprintf(labelstr, "%d %s w: %d", k, type, c_width);
                } 
                if (k==1) {
                    t_width = b_width * 3.1412;
                    //sprintf(labelstr, "%d %s w: %d", k, type, t_width);
                    
                }

                if (c_width > 0 && c_width < 1000) {
                    per_circum = (double)c_width/t_width;
                }
                ...
                printf("t_width: %d\n", t_width);
                printf("per_circum: %d\n", per_circum);
                sprintf(labelstr, "%d %s w: %d %%circum: %lf", k, type, b_width, per_circum); //nice, this outputs item detected, and width and height
